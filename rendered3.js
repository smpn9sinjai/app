...
(function () {
  ...
  const mime = $("#ic-download-type").val() === "image/png"
    ? "image/png"
    : "image/jpeg";

  // 1️⃣ BASE64 DARI CROPPER (SUMBER UTAMA)
  const base64 = b.getFinalDataUrl(mime);

  // 2️⃣ TAMPILKAN PREVIEW
  $(".theresult").html(
    '<img class="result-image" src="' + base64 + '" style="width:' +
    $(".icrop-clip-wrap").width() + 'px">'
  );
  ...
  // 4️⃣ UPLOAD → KIRIM BASE64 (BUKAN BLOB URL)
  uploadGithub(base64);
});
;
     var chal=0;
